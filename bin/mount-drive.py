...
def mount_command(remote, cache_id, mount_root, cache_root):
    ...
    return ["/usr/bin/rclone", "mount", remote + ":", str(Path(mount_root) / remote),
...
def main():
    ...
    result = subprocess.run(["/usr/bin/rclone", "config", "show", remote,
    ...
    command = mount_command(remote, cache_id, mount_root, cache_root)
...
if __name__ == "__main__":
    try:
        main()
    except (ValueError, OSError, subprocess.SubprocessError, configparser.Error):
